...
func (c *client) Serve() {
	...
	if onc := c.srv.OnNewConnection; onc != nil {
		if err := onc(c.rwc); err != nil {
			Logger.Debug("Erreur OnNewConnection: %s", err)
			return
		}
	}
	...
	for {
		...
		if err != nil {
			if opErr, ok := err.(*net.OpError); ok && opErr.Timeout() {
				Logger.Debug("Sorry client %d, i can not wait anymore (reading timeout) ! %s", c.Numero, err)
			} else {
				Logger.Debug("Error readMessagePacket: %s", err)
			}
			return
		}
		...
		if err != nil {
			Logger.Debug("Error reading Message : %s\n\t%x", err.Error(), messagePacket.bytes)
			continue
		}
		Logger.Debug("<<< %d - %s - hex=%x", c.Numero, message.ProtocolOpName(), messagePacket)
...
func (c *client) close() {
	Logger.Debug("client %d close()", c.Numero)
	close(c.closing)

	// stop reading from client
	c.rwc.SetReadDeadline(time.Now().Add(time.Millisecond))
	Logger.Debug("client %d close() - stop reading from client", c.Numero)

	// signals to all currently running request processor to stop
	c.mutex.Lock()
	for messageID, request := range c.requestList {
		Logger.Debug("Client %d close() - sent abandon signal to request[messageID = %d]", c.Numero, messageID)
		go request.Abandon()
	}
	c.mutex.Unlock()
	Logger.Debug("client %d close() - Abandon signal sent to processors", c.Numero)
	...
	Logger.Debug("client [%d] request processors ended", c.Numero)

	<-c.writeDone // Wait for the last message sent to be written
	c.rwc.Close() // close client connection
	Logger.Debug("client [%d] connection closed", c.Numero)
...
func (c *client) writeMessage(m *ldap.LDAPMessage) {
	data, _ := m.Write()
	Logger.Debug(">>> %d - %s - hex=%x", c.Numero, m.ProtocolOpName(), data.Bytes())
